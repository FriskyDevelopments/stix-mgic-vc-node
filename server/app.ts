...
export function createApp() {
  ...
  // Register both `/v1/rooms` and `/v1/rooms/*` so POST /v1/rooms gets an operator id.
  const attachRoomOperator = async (c: Context<{ Variables: Variables }>, next: Next) => {
    const header = c.req.header('authorization') || ''
    const token = header.startsWith('Bearer ') ? header.slice(7) : ''
  ...
  app.use('/v1/rooms', attachRoomOperator)
  app.use('/v1/rooms/*', attachRoomOperator)
  ...
  app.use('/v1/rtmp/*', async (c, next) => {
    ...
    const token = header.startsWith('Bearer ') ? header.slice(7) : ''
    const cookieClaims = !token ? sessionClaimsFromCookie(c.req.header('cookie')) : null
    if (cookieClaims) { c.set('operatorId', cookieClaims.sub); await next(); return }
    if (!token) return c.json({ error: 'Operator token required' }, 401)
    const frisky = verifyFriskyDevToken(token)
    const operator = verifyOperatorToken(token)
    if (frisky || operator) { c.set('operatorId', frisky ? `friskydev:${frisky.sub}` : operator!.sub); await next(); return }
    return c.json({ error: 'Invalid or expired operator token' }, 401)
  })

  app.get('/v1/rtmp/publish', async (c) => {
    try { await assertTelegramNodeOwner(c.get('operatorId')) }
    catch { return c.json({ error: 'Only the connected Telegram account owner can access broadcast settings.' }, 403) }
    const config = getRtmpPublishConfig()
    return config.ready ? c.json(config) : c.json({ error: 'RTMP ingest is not configured' }, 503)
  })

  // Shared account setup and broadcast controls belong to the verified node owner.
  // Group moderation below is separately checked against each caller's Telegram rights.
  app.use('/v1/telegram-vc/*', async (c, next) => {
    const path = c.req.path.slice('/v1/telegram-vc'.length)
    if (path.startsWith('/pair/') || ['/status', '/join', '/leave', '/source'].includes(path)) {
      try { await assertTelegramNodeOwner(c.get('operatorId')) }
      catch { return c.json({ error: 'Only the connected Telegram account owner can change broadcast or account settings.' }, 403) }
    }
    await next()
  })
