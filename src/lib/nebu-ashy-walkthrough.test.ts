...
describe('NEBU Ashy walkthrough step model', () => {
  ...
  it('keeps the three pipes labeled separately', () => {
    ...
    expect(PIPE_LABELS.telegram_broadcast.title).toBe('Telegram dens')
  ...
  it('documents BYO vs hosted with Ashy unit id only as display', () => {
    ...
    expect(unit?.body).toMatch(/api_id \+ api_hash/)
    expect(unit?.body).toMatch(/phone pairing/)
    expect(unit?.body).toMatch(/my\.telegram\.org/)
    expect(unit?.body).toMatch(/BotFather/)
    expect(unit?.body).toMatch(/never paste session bytes/i)
    expect(unit?.secondaryCta?.label).toMatch(/BYO Telethon/)
    expect(unit?.secondaryCta?.note).toMatch(/api_id \+ api_hash \+ phone pairing/)
    expect(unit?.secondaryCta?.note).toMatch(/vault|Wrangler/i)
    expect(unit?.primaryCta?.note).toMatch(/Telethon session/)
  })
  ...
  it('points Connect Telethon at the for-dummies script and manual, never secrets', () => {
    ...
    expect(connect?.secondaryCta?.note).toMatch(/api_id \+ api_hash \+ phone pairing/)
    expect(connect?.secondaryCta?.note).toMatch(/vault|wrangler|\.dev\.vars/i)
    expect(connect?.body).toMatch(/not BotFather/i)
    expect(connect?.body).toMatch(/never paste session bytes/i)
    expect(connect?.primaryCta?.href).toBe(ASHY_UNIT_PATH)
    expect(connect?.primaryCta?.note).toMatch(/FriskyDev-run Telethon session/)
    expect(connect?.primaryCta?.note).toMatch(/never paste session bytes/i)
    const blob = JSON.stringify(connect)
    expect(blob).not.toMatch(/\d{10}:[A-Za-z0-9_-]{20,}/)
  })
})
