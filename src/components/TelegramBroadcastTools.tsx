...
export function TelegramBroadcastTools({ groupId }: { groupId: string }) {
  ...
  const [chatId, setChatId] = useState(groupId)
  useEffect(() => setChatId(groupId), [groupId])
  ...
  // Telegram broadcast adapter — polls /v1/telegram-vc/status; if the server returns 503
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      if (s.call.state === 'active') {
        const p = await getParticipants(s.call.chatId || groupId)
      ...
      if (e?.status === 503 || e?.status === 404 || e?.status === 403) {
        setUnavailable(true)
  ...
  async function applyVcNodeRtmp() {
    ...
    try {
      ...
      setSourceKind('rtmp')
      toast.success('VC Node RTMP endpoint ready', { description: 'Paste it in OBS or join it directly to Telegram broadcast.' })
    } catch (err) {
  ...
  async function handleMute(participantId: string, name: string) {
    try {
      await muteParticipant(chatId, participantId, (await getParticipants(chatId)).callId)
    ...
    return (
      <GlassCard className="p-5" data-testid="telegram-access-preview">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="font-mono text-sm text-cyan-300">TELEGRAM BROADCAST</p>
    ...
    return (
      <GlassCard className="p-4">
        <p className="text-xs text-muted-foreground leading-relaxed">
          Broadcast settings are available to the owner of the connected Telegram account.
          Group moderation is controlled in My groups above.
        </p>
