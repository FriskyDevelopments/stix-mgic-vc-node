...
import { ObsQuickStart } from '@/components/ObsQuickStart'
...
type Props = {
  ...
  onSpotifyPlaybackChange?: (state: SpotifyPlaybackState | null) => void
}
...
export function CreatorTools({ cameraStream, screenStream, onScreenStream, roomId, spotifyAccessToken, onSpotifyDisconnect, onSpotifyPlaybackChange, recordingStream }: Props) {
  ...
  function startRecording() {
    const source = recordingStream === undefined ? screenStream || cameraStream : recordingStream
    if (!source) { toast.error('Prepare an output in the studio monitor first'); return }
  ...
  return (
    <section className="creator-tools grid gap-4 md:grid-cols-2">
      <MediaDisclosure id="spotify-controls" title="Spotify" status={spotifyAccessToken ? "Connected" : undefined}>
      <SpotifyPlayer accessToken={spotifyAccessToken} onDisconnect={onSpotifyDisconnect} onPlaybackChange={onSpotifyPlaybackChange} />
      </MediaDisclosure>

      <MediaDisclosure title="OBS" status={obs.streaming ? "Streaming" : obs.recording ? "Recording" : obs.connected ? "Connected" : undefined}><GlassCard className="p-5">
      ...
      <MediaDisclosure id="capture-controls" title="Capture & record" status={recording ? "Recording" : screenStream ? "Screen preview ready" : undefined}><GlassCard className="p-5"><h2 className="font-mono text-sm text-cyan-300">CAPTURE + RECORD</h2><p className="mt-2 text-xs text-white/50">Choose a screen/window and optionally include system audio. Preview it in the studio monitor, then choose Share preview with room. Files stay on this device.</p><div className="mt-4 flex flex-wrap gap-2"><Button variant="outline" onClick={() => void shareScreen()}>{screenStream ? 'Stop screen preview' : 'Choose screen + audio'}</Button><Button disabled={recordingStream === undefined ? !cameraStream && !screenStream : !recordingStream} onClick={recording ? stopRecording : startRecording}>{recording ? 'Stop & download' : 'Record output locally'}</Button></div><p className="mt-3 text-[10px] text-white/35">Uses MP4 when the browser exposes a native H.264/AAC recorder; otherwise it downloads WebM. Nothing is uploaded.</p></GlassCard></MediaDisclosure>

      <MediaDisclosure title="Invite & calendar"><GlassCard className="p-5"><h2 className="font-mono text-sm text-cyan-300">INVITE + CALENDAR</h2><p className="mt-2 text-xs text-white/50">Create the room once, then send the same secure invite through Telegram or a calendar event.</p><Input className="mt-4" type="datetime-local" value={scheduleAt} onChange={(e) => setScheduleAt(e.target.value)} /><div className="mt-3 flex flex-wrap gap-2"><Button variant="outline" disabled={!roomId} onClick={() => void scheduleInvite('google')}>Google Calendar</Button><Button variant="outline" disabled={!roomId} onClick={() => void scheduleInvite('ics')}>Download .ics</Button>{roomId && <Button onClick={() => { const invite = `${window.location.origin}/?room=${encodeURIComponent(roomId)}`; shareToTelegram(invite, 'Join my scheduled VC Node room') }}>Send to Telegram</Button>}</div></GlassCard></MediaDisclosure>
    </section>
  )
}
